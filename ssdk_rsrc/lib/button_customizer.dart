...
class ButtonParams {
  ...
  ButtonParams({
    ...
    this.focusColor = Colors.lightBlueAccent,
    this.shadowColor = Colors.black26,
    this.shadowOffset = const Offset(2, 2),
    this.isLoading = false,
    this.fontFamily = 'Roboto',
    this.backgroundAlpha = 1.0, // Default opacity
  });
  ...
  static Color _parseColor(String colorString) {
    final regex = RegExp(r'Color\(alpha: (\d?.\d+), red: (\d?.\d+), green: (\d?.\d+), blue: (\d?.\d+)');
    final match = regex.firstMatch(colorString);
    if (match != null) {
      final alpha = (double.parse(match.group(1)!) * 255).toInt();
      final red = (double.parse(match.group(2)!) * 255).toInt();
      final green = (double.parse(match.group(3)!) * 255).toInt();
      final blue = (double.parse(match.group(4)!) * 255).toInt();
  ...
  static EdgeInsetsGeometry _parseEdgeInsets(String paddingString) {
    final regex = RegExp(r'EdgeInsets\((\d+.?\d*), (\d+.?\d*), (\d+.?\d*), (\d+.?\d*)\)');
  ...
  static IconData? _parseIconData(String iconString) {
    ...
    // Add additional parsing logic if required
